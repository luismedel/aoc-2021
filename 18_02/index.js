...
const debug = () => {};
const pair2s = (pair) => isNumber (pair) ? pair : `[${pair.left},${pair.right}]`;
...
class Expression
{
	...
	clone ()
	{
		return new Expression (this.root.clone ());
	}
	...
	findPair (fn)
	{
		...
		pairs.sort ((left, right) => left.stringOffset[0] - right.stringOffset[0]);
		return pairs[0];
	}
	...
	{
		return new Expression (new Pair (left.root.clone (), right.root.clone ()));
	}
...
class Pair
{
	...
	clone ()
	{
		return new Pair (isNumber (this.left)  ? this.left  : this.left.clone (),
...
for (let i = 0; i < exprs.length - 1; i++)
{
	...
	for (let j = i + 1; j < exprs.length; j++)
	{
		...
		max = Math.max (Expression.add (left, right).fullReduce ().getMagnitude (),
						Expression.add (right, left).fullReduce ().getMagnitude (),
						max);
	}
}
